    ...
    Control {
        ...
        contentItem: Flickable {
            ...
            ColumnLayout {
                ...
                // Wallpaper Properties panel — reads project.json and renders per-property controls.
                // IMPORTANT: ListModel and Component items cannot be children of OptionGroup because
                // OptionGroup.content aliases ColumnLayout.children (QQuickItem only). They live here
                // as siblings inside flick_content (ColumnLayout.data accepts any QObject).
                ListModel { id: wp_props_model }
                Component {
                    id: prop_switch
                    Switch {
                        signal propChanged(var v)
                        function initProp(m, val) { checked = !!val; }
                        onCheckedChanged: propChanged(checked)
                    }
                }
                Component {
                    id: prop_spinbox
                    SpinBox {
                        signal propChanged(var v)
                        function initProp(m, val) {
                            from = Math.round(m.min); to = Math.round(m.max);
                            stepSize = Math.max(1, Math.round(m.step));
                            value = Math.round(val);
                        }
                        onValueModified: propChanged(value)
                    }
                }
                Component {
                    id: prop_dspinbox
                    DoubleSpinBox {
                        signal propChanged(var v)
                        function initProp(m, val) {
                            dFrom = m.min; dTo = m.max; dStepSize = m.step || 0.01;
                            dValue = val;
                        }
                        onDValueChanged: propChanged(dValue)
                    }
                }
                Component {
                    id: prop_combo
                    ComboBox {
                        ...
                        function initProp(m, val) {
                            _opts = m.options;
                            model = m.options.map(function(o) { return o.label || String(o.value); });
                            var idx = m.options.findIndex(function(o) { return o.value == val; });
                            currentIndex = idx >= 0 ? idx : 0;
                        }
                        onActivated: propChanged(_opts[currentIndex] ? _opts[currentIndex].value : currentIndex)
                    }
                }
                Component {
                    id: prop_color
                    TextField {
                        signal propChanged(var v)
                        placeholderText: "r g b  (0-1)"
                        function initProp(m, val) { text = val || '1 1 1'; }
                        onEditingFinished: propChanged(text)
                    }
                }
                ...
                OptionGroup {
                    ...
                    function reload() {
                        wp_props_model.clear();
                        const projPath = Common.getWpModelProjectPath(right_content.wpmodel);
                        if (!projPath) return;
                        pyext.readfile(Common.urlNative(projPath)).then(function(text) {
                            const proj = Utils.parseJson(text);
                            ...
                            for (var key in props) {
                                ...
                                if (p.type === 'text' || p.type === 'file') continue;
                                items.push({
                                    key:       key,
                                    label:     (p.text || key).replace(/^ui_/, '').replace(/_/g, ' '),
                                    type:      p.type,
                                    defVal:    p.value !== undefined ? p.value : (p.type === 'bool' ? false : 0),
                                    min:       p.min       !== undefined ? p.min       : 0,
                                    max:       p.max       !== undefined ? p.max       : 100,
                                    step:      p.step      !== undefined ? p.step      : 1,
                                    precision: p.precision !== undefined ? p.precision : 0,
                                    options:   p.options   || []
                                });
                            }
                            items.sort(function(a, b) { return a.label.localeCompare(b.label); });
                            for (var i = 0; i < items.length; i++)
                                wp_props_model.append(items[i]);
                        }).catch(function(e) { console.error("wp_props reload error", e); });
                    }

                    Repeater {
                        model: wp_props_model
                        delegate: RowLayout {
                            Layout.fillWidth: true
                            spacing: 12
                            ...
                            Loader {
                                sourceComponent: {
                                    if (model.type === 'bool')  return prop_switch;
                                    if (model.type === 'color') return prop_color;
                                    if (model.type === 'combo') return prop_combo;
                                    if (model.precision === 0)  return prop_spinbox;
                                    return prop_dspinbox;
                                }
                                onLoaded: {
                                    const key = model.key;
                                    const saved = right_opts.get_config_val(key);
                                    const val = (saved !== null) ? saved : model.defVal;
                                    item.initProp(model, val);
                                    item.onPropChanged.connect(function(v) {
                                        right_opts.set_config(key, v);
                                    });
                                }
                            }
                        }
